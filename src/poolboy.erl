...
-export([checkout/1, checkout/2, checkout/3, checkin/2, transaction/2,
         ...
         start_link/1, start_link/2, stop/1, status/1, estatus/1]).
...
-spec estatus(Pool :: pool()) -> {atom(), integer(), integer(), integer()}.
estatus(Pool) ->
    gen_server:call(Pool, estatus).
...
handle_call(estatus, _From, State) ->
    #state{workers = Workers,
           monitors = Monitors,
           overflow = Overflow,
